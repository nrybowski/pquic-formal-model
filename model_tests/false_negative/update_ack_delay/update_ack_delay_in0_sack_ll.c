...
protoop_arg_t update_ack_delay (picoquic_cnx_t *cnx) {
    ...
    picoquic_sack_item_t *runner = (picoquic_sack_item_t*) get_pkt_ctx(pkt_ctx, AK_PKTCTX_FIRST_SACK_ITEM), *memory = NULL;
    if (runner) {
        while (runner != NULL) {
            memory = runner;
            runner = (picoquic_sack_item_t*) get_sack_item(runner, AK_SACKITEM_NEXT_SACK);
        }

        // memory should be the last element of the linked list
        set_sack_item(memory, AK_SACKITEM_START_RANGE, 0);
    }
