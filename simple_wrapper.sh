...
RUN echo horn | sudo -S DEBIAN_FRONTEND=noninteractive su -p - -c "apt-get install -y --no-install-recommends --fix-missing libarchive-dev libssl-dev"
EOF
...
BPF_OPTS="-m64 -g --bmc=mono --inline --track=mem --dsa=sea-cs \
	...
	-I/opt/picoquic -DDISABLE_PROOTOP_PRINTF -DDISABLE_QLOG /opt/model/verifier/verifier.c /opt/picoquic/*.c"
